...
const parseDayKeyDayjsStrict = (dayKey: DayKey, timeZone: string) => {
  ...
  const parsed = (dayjs as typeof dayjs & { tz: DayjsTzStrictParser }).tz(dayKey, DAY_KEY_FORMAT, timeZone, true);
...
export const getDayInterval = (date: Date, timeZone: string) =>
  getDayIntervalForDayKey(formatDayKey(date, timeZone), timeZone);
